...
var commentList = [
  {
    ...
    comments: [
      ...
      },
      {
        author: '方荣甫',
        content:
          '信息检索起源于图书馆的参考咨询和文摘索引工作，目的在于从一大推信息中找到我们所需要的某部分思想，进一步' +
      ...
      <div className={styles.CommentView} key={index}>
        ...
        <div className={styles.addCommentView} hidden={item.addCommentDisabled === '0'} key={index}>
          ...
          <div className={styles.buttonView}>
            <button className={styles.button1} onClick={() => cancelAddComment(index)}>
              取消
            </button>
            <button className={styles.button2} onClick={() => confirmAddComment(index)}>
              确定
            </button>
          </div>
        </div>
